...
public class DownStreamSystemService {
    ...
    private volatile ConcurrentHashMap<String, FutureTask<Long>> taskMap = new ConcurrentHashMap<>();

    public FutureTask<Long> cal(Long id, String redisKey) {
        FutureTask<Long> task = null;
        try {
           task = new FutureTask(new CalCallable(id));
           executorService.execute(task);
           taskMap.put(redisKey, task);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public FutureTask<Long> getTaskUsingRedisKey(String redisKey) {
        return taskMap.get(redisKey);
    }
